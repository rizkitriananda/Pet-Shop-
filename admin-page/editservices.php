...
$data = query("SELECT * FROM services WHERE id = $id")[0];

// cek apakah tombol submit sudah ditekan atau belum
if(isset($_POST['submit'])) {
    // cek apakah data berhasil diubah atau tidak
    if(ubahServices($_POST) > 0 ){
        echo "<script>
      ...
      <section class="mt-[80px] px-3 mx-auto flex flex-col justify-center">
        ...
        <form action="" method="post" enctype="multipart/form-data" class="bg-[#fefefe] w-[500px] p-7 rounded-lg flex flex-col">
          ...
          <div class="flex justify-between mb-3">
            <label for="hewan" class="self-center">Deskripsi</label>
            <input type="text" name="deskripsi" id="hewan" value="<?= $data['deskripsi']; ?>" autofocus class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="flex justify-between mb-3">
            <label for="brand" class="self-center">Layanan</label>
            <input type="text" name="layanan" id="brand" value="<?= $data['layanan']; ?>" class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="flex justify-between mb-3">
            <label for="stok" class="self-center">Harga</label>
            <input type="text" name="harga" id="stok" value="<?= $data['harga']; ?>" class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="h-[50px] flex items-center justify-between mb-3 relative overflow-hidden">
            <label for="gambar">Gambar</label>
            <img src="../image/services/<?= $data['gambar']; ?>" alt="" class="w-[50px] absolute left-[85px]" />
