...
$data = query("SELECT * FROM foods WHERE id = $id")[0];
    ...
    <main class="flex">
      <section class="mt-[80px] px-3 mx-auto flex flex-col justify-center">
        ...
        <form action="" method="post" enctype="multipart/form-data" class="bg-[#fefefe] w-[500px] p-7 rounded-lg flex flex-col">
          <input type="number" name="id" id="" class="hidden" value="<?= $data['id']; ?>"/>
          <input type="hidden" name="gambarLama" value="<?= $data['gambar']?>">

          <div class="flex justify-between mb-3">
            <label for="hewan" class="self-center">Hewan</label>
            <input type="text" name="hewan" id="hewan" value="<?= $data['hewan']; ?>" autofocus class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="flex justify-between mb-3">
            <label for="brand" class="self-center">Brand</label>
            <input type="text" name="brand" id="brand" value="<?= $data['brand']; ?>" class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="flex justify-between mb-3">
            <label for="stok" class="self-center">Stok</label>
            <input type="text" name="stok" id="stok" value="<?= $data['stok']; ?>" class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="h-[50px] flex items-center justify-between mb-3 relative overflow-hidden">
            <label for="gambar">Gambar</label>
            <img src="../image/foods/<?= $data['gambar']; ?>" alt="" class="w-[50px] absolute left-[85px]" />
            <input type="file" name="gambar" id="gambar" class="absolute -right-14" />
          </div>

          <div class="flex justify-between mb-3">
            <label for="" class="self-center">Harga</label>
            <input type="text" name="harga" id="harga" value="<?= $data['harga']; ?>" class="px-3 py-[2px] w-[250px] rounded focus:outline-[#830FF3] border border-[#777]" />
          </div>

          <div class="flex justify-between mb-3">
            <label for="">Deskripsi</label>
            <textarea name="deskripsi" id=""  cols="30" rows="10" class="rounded w-[250px] px-2 focus:outline-[#830FF3] resize-none border border-[#777]"><?= $data['deskripsi']; ?></textarea>
          </div>
